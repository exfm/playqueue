...
class AudioManager {
  ...
  // Handles notifyBeforeEnd and notifySongHalf
  timeUpdate() {
    if (this.notifyBeforeEnd === true 
  ...
  next(e) {
    // not user initiated
    if (e && e.type === 'ended') {
      if (this.listManager.queueNumber < this.listManager.length - 1 && this.listManager.autoNext === true) {
        this._next();
      } else {
        this.stop();
      } 
      ...
      // user initiated
      if (this.listManager.queueNumber < this.listManager.length - 1) {
        this._next();
      } else {
  ...
  _next(e) {
    this.listManager.queueNumber = this.listManager.queueNumber + 1;
    this.play(this.listManager.queueNumber);
    this.eventBus.trigger('nextTrack', {
      'song': song,
      'queueNumber': this.listMananger.queueNumber,
      'audio': this.audioProperties
    });
  ...
  previous() {
    if (this.listManager.smartPrevious === true) {
      ...
      } else if (this.listManager.queueNumber > 0) {
        this._previous();
      }
    } else if (this.listManager.queueNumber > 0) {
      this._previous();
  ...
  _previous() {
    this.listManager.queueNumber = this.listManager.queueNumber - 1;
    this.play(this.listManager.queueNumber);
    this.eventBus.trigger('previousTrack', {
      'song': song,
      'queueNumber': this.listMananger.queueNumber,
      'audio': this.audioProperties
    });
  ...
  stop() {
    this.isStopped = true;
    this.listManager.queueNumber = 0;
 ...
 */
 ...
 // todo - create a state object with song, audio, isStopped, queueNumber, shuffle, etc